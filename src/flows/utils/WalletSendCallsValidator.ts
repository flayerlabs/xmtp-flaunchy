...
export function validateWalletSendCalls(
  walletSendCalls: any
): walletSendCalls is WalletSendCallsStructure {
  if (!walletSendCalls || typeof walletSendCalls !== "object") {
    console.error(
      "WalletSendCalls validation failed: object is null, undefined, or not an object",
      walletSendCalls
    );
    return false;
  }
  ...
  if (!walletSendCalls.calls || !Array.isArray(walletSendCalls.calls)) {
    console.error(
      "WalletSendCalls validation failed: missing or invalid calls array",
      walletSendCalls
    );
    return false;
  }
  ...
  if (walletSendCalls.calls.length === 0) {
    console.error(
      "WalletSendCalls validation failed: calls array is empty",
      walletSendCalls
    );
    return false;
  }
  ...
  for (let i = 0; i < walletSendCalls.calls.length; i++) {
    ...
    if (!call || typeof call !== "object") {
      console.error(
        `WalletSendCalls validation failed: call ${i} is not an object`,
        call
      );
      return false;
    }
    ...
    if (typeof call.chainId !== "number") {
      console.error(
        `WalletSendCalls validation failed: call ${i} missing or invalid chainId`,
        call
      );
      return false;
    }
    ...
    if (typeof call.to !== "string" || !call.to) {
      console.error(
        `WalletSendCalls validation failed: call ${i} missing or invalid to address`,
        call
      );
      return false;
    }
    ...
    if (typeof call.data !== "string" || !call.data) {
      console.error(
        `WalletSendCalls validation failed: call ${i} missing or invalid data`,
        call
      );
      return false;
    }
    ...
    if (typeof call.value !== "string") {
      console.error(
        `WalletSendCalls validation failed: call ${i} missing or invalid value`,
        call
      );
      return false;
    }
  ...
  if (typeof walletSendCalls.version !== "string" || !walletSendCalls.version) {
    console.error(
      "WalletSendCalls validation failed: missing or invalid version",
      walletSendCalls
    );
    return false;
  }
  ...
  if (typeof walletSendCalls.from !== "string" || !walletSendCalls.from) {
    console.error(
      "WalletSendCalls validation failed: missing or invalid from address",
      walletSendCalls
    );
    return false;
  }
  ...
  if (typeof walletSendCalls.chainId !== "string" || !walletSendCalls.chainId) {
    console.error(
      "WalletSendCalls validation failed: missing or invalid chainId",
      walletSendCalls
    );
    return false;
  }
...
export async function safelySendWalletSendCalls(
  ...
  if (!validateWalletSendCalls(walletSendCalls)) {
    console.error(
      "Refusing to send invalid wallet send calls to prevent UI crash"
    );
    return false;
  }
