...
export class OnboardingFlow extends BaseFlow {
  ...
  async processMessage(context: FlowContext): Promise<void> {
    ...
    if (userState.status === 'onboarding' && messageText) {
      const wantsNewGroup = await this.detectNewGroupCreation(context, messageText);
      if (wantsNewGroup) {
        await this.handleGroupCreation(context);
        return;
      }
    ...
    if (userState.status === 'onboarding') {
      ...
      if (userState.pendingTransaction && messageText) {
        const isFeeSplitModification = await this.detectFeeSplitModification(context, messageText);
      ...
      await this.sendResponse(context, "you're all set up! create more groups or launch coins anytime.");
    }
  }
  ...
  private async handleGroupCreation(context: FlowContext): Promise<void> {
    ...
    const isAskingAboutExisting = await this.detectExistingReceiversInquiry(context, messageText);
          ...
          if (errorMessage.includes('Total shares') && errorMessage.includes('do not equal required total')) {
            ...
            try {
              ...
              if (extraction && extraction.receivers.length > 0) {
                const resolvedReceivers = await GroupCreationUtils.resolveUsernames(context, extraction.receivers);
                ...
                const totalUsedPercentage = resolvedReceivers.reduce((sum, r) => sum + (r.percentage || 0), 0);
                ...
                const receiverNames = resolvedReceivers
                  .map(r => r.username && r.username.startsWith('@') ? r.username : `@${r.username}`)
                  .join(', ');
                
                await this.sendResponse(context, `got it! ${receiverNames} will get ${totalUsedPercentage}% of trading fees. who else should get the remaining ${remainingPercentage}%?`);
                return;
              }
            } catch (extractError) {
              this.logError('Failed to extract partial receivers', extractError);
            }
          }
        }
      }
  ...
  private async handleCoinLaunch(context: FlowContext): Promise<void> {
    ...
    if (context.userState.status === 'onboarding' && 
  ...
  private async addToExistingGroup(context: FlowContext, messageText: string): Promise<void> {
    try {
      ...
      try {
        ...
        await this.sendResponse(context, "couldn't understand who to add. try again with @username or address.");
        return;
      }
      ...
      let newReceivers;
      try {
        newReceivers = await GroupCreationUtils.resolveUsernames(context, extraction.receivers);
      } catch (error) {
        this.logError('Failed to resolve usernames', error);
        await this.sendResponse(context, "couldn't resolve usernames. please try again.");
        return;
      }
      ...
      if (failed.length > 0) {
        await this.sendResponse(context, `couldn't resolve these usernames: ${failed.map(r => r.username).join(', ')}`);
        return;
      }
      ...
      if (newReceivers.length > 0) {
        ...
        if (newReceiversWithPercentage.length > 0) {
          ...
          if (totalSpecifiedPercentage >= 100) {
            await this.sendResponse(context, "specified percentage is too high. please use a lower percentage to leave room for existing members.");
            return;
          }
          ...
          if (existingReceiversWithPercentage.length > 0) {
            ...
            if (remainingPercentage <= 0) {
              await this.sendResponse(context, "existing receivers already use 100% of fees. please adjust percentages or replace some receivers.");
              return;
            }
  ...
  private async detectNewGroupCreation(context: FlowContext, messageText: string): Promise<boolean> {
    if (!messageText) return false;
  ...
  private async detectFeeSplitModification(context: FlowContext, messageText: string): Promise<boolean> {
    if (!messageText) return false;
  ...
  private async handleFeeSplitModification(context: FlowContext): Promise<void> {
    const messageText = this.extractMessageText(context);
    const { userState } = context;
    ...
    try {
      ...
      if (result && result.resolvedReceivers.length > 0) {
        ...
        if (userState.pendingTransaction.type === 'group_creation') {
          ...
          await this.sendResponse(context, "fee split modifications for coin transactions not yet supported.");
        }
      } else {
        await this.sendResponse(context, "couldn't parse the fee split modification. please specify usernames and percentages.");
      }
    } catch (error) {
      this.logError('Fee split modification error', error);
      await this.sendResponse(context, "couldn't modify fee splits. please try again with clear usernames and percentages.");
    }
  }

  private async detectPercentageUpdate(context: FlowContext, messageText: string): Promise<boolean> {
    if (!messageText) return false;
  ...
  private async handlePercentageUpdate(context: FlowContext, messageText: string): Promise<void> {
    try {
      // Get existing receivers from pending transaction
      const existingReceivers = context.userState.onboardingProgress?.splitData?.receivers || [];
      
      if (existingReceivers.length === 0) {
        await this.sendResponse(context, "no existing receivers to update. please create the group first.");
        return;
      }
      ...
      try {
        ...
        await this.sendResponse(context, "couldn't understand the percentage update. try something like 'give @alice 50%'.");
        return;
      }

      if (!extraction || !extraction.receivers || extraction.receivers.length === 0) {
        await this.sendResponse(context, "couldn't find any percentage updates. try something like 'give @alice 50%'.");
        return;
      }
      ...
      let updatedReceivers;
      try {
        updatedReceivers = await GroupCreationUtils.resolveUsernames(context, extraction.receivers);
      } catch (error) {
        this.logError('Failed to resolve usernames for percentage update', error);
        await this.sendResponse(context, "couldn't resolve usernames. please try again.");
        return;
      }
      ...
      const receiversWithPercentage = updatedReceivers.filter(r => r.percentage !== undefined);
      
      if (receiversWithPercentage.length === 0) {
        await this.sendResponse(context, "no percentage specified. try something like 'give @alice 50%'.");
        return;
      }

      // Calculate total specified percentage
      const totalSpecifiedPercentage = receiversWithPercentage.reduce((sum, r) => sum + (r.percentage || 0), 0);
      
      if (totalSpecifiedPercentage >= 100) {
        await this.sendResponse(context, "specified percentage is too high. please use a lower percentage to leave room for other members.");
        return;
      }
      ...
      const finalReceivers = existingReceivers.map(existing => {
        ...
        const update = receiversWithPercentage.find(updated => 
          updated.resolvedAddress?.toLowerCase() === existing.resolvedAddress?.toLowerCase() ||
          (updated.username && updated.username.toLowerCase() === existing.username?.toLowerCase())
        );
      ...
      const remainingPercentage = 100 - totalSpecifiedPercentage;
      const receiversWithoutPercentage = finalReceivers.filter(r => 
        !receiversWithPercentage.some(updated => 
          updated.resolvedAddress?.toLowerCase() === r.resolvedAddress?.toLowerCase() ||
          (updated.username && updated.username.toLowerCase() === r.username?.toLowerCase())
        )
      );
      
      const equalPercentage = receiversWithoutPercentage.length > 0 ? remainingPercentage / receiversWithoutPercentage.length : 0;

      // Apply equal split to remaining receivers
      const redistributedReceivers = finalReceivers.map(receiver => {
        const hasSpecificPercentage = receiversWithPercentage.some(updated => 
          updated.resolvedAddress?.toLowerCase() === receiver.resolvedAddress?.toLowerCase() ||
          (updated.username && updated.username.toLowerCase() === receiver.username?.toLowerCase())
        );
      ...
      try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        if (errorMessage.includes('Total shares') && errorMessage.includes('do not equal required total')) {
          await this.sendResponse(context, "error updating percentages - they don't add up to 100%. please try again.");
          return;
        } else {
          this.logError('Group deployment error in percentage update', error);
          await this.sendResponse(context, "couldn't update group percentages. please try again.");
          return;
        }
      }
      ...
      await this.sendResponse(context, "couldn't update percentages. please try again.");
    }
  }
   ...
   */
  private extractReceiversFromTransaction(walletSendCalls: any, originalReceivers: any[]): any[] {
    try {
      ...
      if (!walletSendCalls || !walletSendCalls.calls || walletSendCalls.calls.length === 0) {
  ...
  private async createTransactionMessage(
    ...
    const actualReceivers = this.extractReceiversFromTransaction(walletSendCalls, receivers);
    
    // Use shared utility for consistent message formatting
    const baseMessage = GroupCreationUtils.createTransactionMessage(actualReceivers, messageType);
   ...
   */
  private async detectExistingReceiversInquiry(context: FlowContext, messageText: string): Promise<boolean> {
    if (!messageText) return false;
   ...
   */
  private async handleExistingReceiversInquiry(context: FlowContext): Promise<void> {
    const existingReceivers = context.userState.onboardingProgress?.splitData?.receivers || [];
    
    if (existingReceivers.length === 0) {
      await this.sendResponse(context, "no receivers set yet. specify who should receive trading fees by tagging usernames (@alice) or using addresses.");
      return;
    }
    ...
    for (const receiver of existingReceivers) {
      const displayName = receiver.username && receiver.username.startsWith('@') 
        ? receiver.username 
        : `${receiver.resolvedAddress?.slice(0, 6)}...${receiver.resolvedAddress?.slice(-4)}`;
