...
export class ENSResolverService {
  ...
  private async processBatch(addresses: string[], resultMap: Map<string, string>): Promise<void> {
    try {
      const queries = addresses.map(address => `
        query_${address.replace(/[^a-zA-Z0-9]/g, '_')}: enses(addressesOrNames:["${address}"]) {
          id
          address
          displayName
          source
      ...
      if (!response.ok) {
        throw new Error(`ENS resolution request failed: ${response.status} ${response.statusText}`);
      }
  ...
  async resolveSingleAddress(address: string): Promise<string> {
    ...
    return result.get(address.toLowerCase()) || `${address.slice(0, 6)}...${address.slice(-4)}`;
  }
} 